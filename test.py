...
class BaseMigracaoMovimento:
    def __init__(self, engine_new, engine_legado, dados_compartilhados, start_counter=1, limpar_ambiente = False):
        ...
        with self.engine_new.connect() as conn:
            # Pegando os MAX IDs das tabelas de serviço e movimento
            ...
            # Busca usuários válidos para fallback global
        ...
        # 🎯 Contadores de Transporte acoplados ao Pai
        ...
        # 🎯 Listas de Transporte acopladas ao Pai
    ...
    def buscar_equipamentos_novo_por_tombo(self, lista_tombos: list) -> dict:
        ...
        query = f"""
            SELECT eq.id, eq.number, eq.name, eq.last_movement_item_customer_id, eq.deleted_at 
            FROM equipments eq 
    ...
    def registrar_movimento(
        ...
        # 1️⃣ CAPAS PAI (Service Order + Movement)
            self.servicos_mestre.append({
                "id": novo_id_capa, "status_id": 3, "movement_type_id": tipo_movimento_id,
                "contract_id": contrato_id, "user_id": usuario_seguro, "destination_order_id": None,
                "mode_transport_id": 1, "organization_id": organization_id, "recipient_customer_id": recipient_id,
                "deadline": mov_date, "details": details_capa, "created_at": mov_date, "updated_at": mov_date, "deleted_at": deleted_at_mov
            })
            self.movimentos_mestre.append({
                "id": novo_id_capa, "number": id_final, "movement_date": mov_date,
                "service_order_id": novo_id_capa, "recipient_customer_id": recipient_id,
                "migrate_customer_id": None, "organization_id": organization_id, "status_id": 3,
                "created_by": usuario_seguro, "details": details_capa, "created_at": mov_date,
                "updated_at": mov_date, "deleted_at": deleted_at_mov
            })
        ...
        self.service_itens_mestre.append({
            "id": item_servico_id_atual, "status_id": 3, "service_order_id": id_capa_atual,
            ...
            "is_exchange": 1 if is_exchange else 0, "is_extra": is_extra_flag, "quantity_product": None,
        ...
        self.movimento_itens_mestre.append({
            "id": item_mov_id_atual, "movement_id": id_capa_atual, "movement_type_id": tipo_movimento_id,
            "service_order_item_id": item_servico_id_atual, "equipment_id": equipment_id_ref,
            "extra_id": extra_id_atual, "status_id": 3, "product_item_id": None, "alias": alias_movimento,
            "old_organization_id": None, "new_organization_id": organization_id, "operation_type": operation_type,
            "confirmed_at": None, "confirmed_by": None,
            "created_at": mov_date, "updated_at": mov_date, "deleted_at": deleted_at_mov
        })

        # 5️⃣ SHIPMENTS (GUIA DE TRANSPORTE CENTRALIZADA) 🎯
        # Cada vez que um equipamento se move, o Pai gera um Shipment para o endereço de destino (cliente_final_address_id)
    ...
    # ==========================================================================
    # 1. PERSISTÊNCIA DE NOVOS REGISTROS (INSERT EM MASSA UNIFICADO)
    # ==========================================================================
    def salvar_movimentos_banco(self):
        """Salva a estrutura completa (Service, Movements, Extras e Shipments) no banco."""
        print(f"\n🚀 Persistindo infraestrutura completa de movimentação no MySQL...")
        with self.engine_new.begin() as conn:
            ...
            # 🎯 Tabelas de Transporte Unificadas
        ...
        print(f"📦 Itens Movimentados: {len(self.service_itens_mestre)}")
        print(f"🚚 Guias de Transporte (Shipments): {len(self.shipments_mestre)}")
