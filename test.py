...
def carregar_dados_compartilhados(engine_legado, engine_new):
    ...
    with engine_legado.connect() as conn:
        ...
        df_ultimo_mov = pd.read_sql(
            "SELECT equipamento_id, MAX(movimento_id) as max_mov FROM aluguel_movimento_itens GROUP BY equipamento_id", conn
        )
        dict_ultimo_mov_equip = dict(zip(df_ultimo_mov['equipamento_id'].astype(int), df_ultimo_mov['max_mov'].astype(int)))
    ...
    with engine_new.connect() as conn:
        ...
        query_saldos = text("""
            SELECT
                coi.id AS contract_item_id, coi.alias AS contract_item_alias,
                con.id AS contract_id, con.name AS contract_name,
                coi.quantity, coi.available_quantity
        ...
        df_equipamentos_refatorado = pd.read_sql("SELECT id, number, name, current_organization_id, deleted_at FROM equipments", conn)
        ...
        query_primeiro_item = text("""
            ...
            ORDER BY ci.id ASC
        """)
        df_primeiro_item = pd.read_sql(query_primeiro_item, conn).drop_duplicates(subset=['customer_id'], keep='first')
        ...
        """)
        df_primeiro_item_por_contrato = pd.read_sql(query_primeiro_item_por_contrato, conn).drop_duplicates(subset=['contract_id'], keep='first')

        query_refs_equipamentos = text("""
            SELECT
                eqp.id AS equipment_id, prod.id AS product_id, types.id AS type_id
            FROM equipments eqp
        ...
        dict_refs_por_equipamento = {int(row.equipment_id): {'type_id': int(row.type_id), 'product_id': int(row.product_id)} for row in result_refs}

    dict_tombo_por_equip_id = {row['id']: limpar_codigo(row['numero']) for _, row in df_equipamentos_legado.iterrows() if pd.notna(row['numero'])}
    df_equipamentos_ativos = df_equipamentos_refatorado[df_equipamentos_refatorado['deleted_at'].isna()].sort_values('id')
    dict_equip_ref_por_number = {limpar_codigo(row['number']): int(row['id']) for _, row in df_equipamentos_ativos.iterrows()}
    ...
    dict_primeiro_item_por_contrato = dict(zip(df_primeiro_item_por_contrato['contract_id'].astype(int), df_primeiro_item_por_contrato['contract_item_id'].astype(int)))

    dict_contratos_vinculados = df_vinculos.groupby('customer_id')['contract_id'].apply(list).to_dict()

    dict_contrato_item_por_chave = {
        (int(row['cliente_id']), int(row['contract_id']), normalizar_para_match(row['alias_item_contract']), normalizar_para_match(row['description'])): {
        ...
        for _, row in df_contratos_itens.iterrows() if pd.notna(row['alias_item_contract']) and pd.notna(row['cliente_id'])
    }
    ...
    for _, row in df_contratos_itens.iterrows():
        if pd.isna(row['legacy_client_id']) or pd.isna(row['alias_item_contract']): continue
        ...
        if not legacy_client_id or not contract_id or not contract_item_id: continue

        info_item = {'id': contract_item_id, 'contract_id': contract_id, 'available_quantity': limpar_valor_numerico(row['available_quantity']), 'original_quantity': limpar_valor_numerico(row['available_quantity'])}
        chave_contrato = (legacy_client_id, contract_id)
        dict_contrato_aluguel_por_chave.setdefault(chave_contrato, {'contract_id': contract_id, 'first_contract_item_id': dict_primeiro_item_por_contrato.get(contract_id, contract_item_id)})
        chave_item = (legacy_client_id, contract_id, normalizar_para_match(row['alias_item_contract']), normalizar_para_match(row['description']))
        dict_contrato_item_aluguel_por_chave.setdefault(chave_item, info_item)

    saldos_por_id = {int(row['contract_item_id']): limpar_valor_numerico(row['available_quantity']) for _, row in df_saldos_contract_items.iterrows() if pd.notna(row['contract_item_id'])}
    dict_tipo_por_equipamento = {k: v['type_id'] for k, v in dict_refs_por_equipamento.items()}

    print(f"   ✅ {len(dict_contrato_item_por_chave)} combinações indexadas.")
    return {
        "dict_tombo_por_equip_id": dict_tombo_por_equip_id, "dict_equip_ref_por_number": dict_equip_ref_por_number,
        "ids_equipamentos_ref": ids_equipamentos_ref, "dict_movimentos_legado": dict_movimentos_legado,
        "dict_cliente_adress": dict_cliente_adress, "dict_endereco_por_legacy_client": dict_endereco_por_legacy_client,
        "dict_primeiro_item_por_cliente": dict_primeiro_item_por_cliente, "dict_primeiro_contrato_por_cliente": dict_primeiro_contrato_por_cliente,
        "dict_primeiro_item_por_contrato": dict_primeiro_item_por_contrato, "dict_contratos_vinculados": dict_contratos_vinculados,
        "dict_contrato_item_por_chave": dict_contrato_item_por_chave, "dict_contrato_item_aluguel_por_chave": dict_contrato_item_aluguel_por_chave,
        "dict_contrato_aluguel_por_chave": dict_contrato_aluguel_por_chave, "dict_tipo_por_equipamento": dict_tipo_por_equipamento,
        "dict_refs_por_equipamento": dict_refs_por_equipamento, "dict_ultimo_mov_equip": dict_ultimo_mov_equip, "saldos_por_id": saldos_por_id, "df_movimento_item_legado": df_movimento_item_legado,
    }
...
class BaseMigracaoMovimento:
    def __init__(self, engine_new, engine_legado, dados_compartilhados, start_counter=1, limpar_ambiente=False):
        ...
        with self.engine_new.connect() as conn:
            ...
            self.usuarios_validos = set(row[0] for row in conn.execute(text("SELECT id FROM users")).fetchall())
    ...
    def buscar_ultimo_movimento_por_tombo(self, lista_tombos: list) -> dict:
        if not lista_tombos: return {}
        lista_tombos_sql = "(" + ", ".join(map(str, lista_tombos)) + ")"
        query = f"""
            SELECT am.id, am.data, am.tipo_id, amt.nome AS tipo_nome,
                   am.cliente_id, am.usuario_id, am.updated_at, am.deleted_at, ae.id AS equipment_id, ae.numero AS tombo
            ...
            WHERE am.deleted_at IS NULL AND ae.deleted_at IS NULL AND ae.numero IN {lista_tombos_sql}
              AND am.id = (SELECT am2.id FROM aluguel_movimento am2 INNER JOIN aluguel_movimento_itens ami2 ON ami2.movimento_id = am2.id WHERE ami2.equipamento_id = ae.id AND am2.deleted_at IS NULL ORDER BY am2.updated_at DESC, am2.data DESC LIMIT 1)
            ORDER BY ae.numero, am.updated_at DESC, am.data DESC, am.id DESC;
        """
        dict_res = {}
        for _, row in pd.read_sql(query, self.engine_legado).iterrows():
            tombo_chave = limpar_codigo(row['tombo'])
            if tombo_chave and tombo_chave != 'nan': dict_res.setdefault(tombo_chave, {'movimento': row.to_dict(), 'equipment_id': int(row['equipment_id']), 'data_dt': pd.to_datetime(row['updated_at'] if pd.notna(row['updated_at']) else row['data'])})
        return dict_res
    ...
    def buscar_equipamentos_novo_por_tombo(self, lista_tombos: list) -> dict:
        ...
        query = f"SELECT eq.id, eq.number, eq.name, eq.last_movement_item_customer_id, eq.deleted_at FROM equipments eq WHERE eq.number IN {lista_tombos_sql} AND eq.deleted_at IS NULL"
        return {limpar_codigo(row['number']): int(row['id']) for _, row in pd.read_sql(text(query), self.engine_new).iterrows() if limpar_codigo(row['number'])}

    def calcular_saldo(self, contrato_item_id, recipient_id, equipment_id_ref, mov_date, item_servico_id_atual, fallback_contract_item_id=None, forcar_extra=False):
        return 0, None, contrato_item_id

    # 🎯 CORREÇÃO DO NOME AQUI: Volta para aplicar_regras_comerciais_parquet
    def aplicar_regras_comerciais_parquet(self, csv_contract_id, csv_item_id, equipment_id_ref, recipient_id, dict_is_kit, abater_saldo=True):
    ...
    def registrar_movimento(
        self, id_final: int, recipient_id: int, cliente_final_address_id: int, usuario_id: int, mov_date: str, deleted_at_mov: str,
        contrato_id: int, organization_id: int, contrato_item_id: int, equipment_id_ref: int, tipo_movimento_id: int, status_shipment: int,
        operation_type: str, status_equipment_id: int, history_reason: str, type_id_ref: int = None, product_id_ref: int = None,
        alias_item: str = None, alias_movimento: str = None, details_capa: str = "Processo de Migração", details_item: str = None,
        fallback_contract_item_id: int = None, forcar_extra: bool = False, is_exchange: bool = False, consumir_saldo: bool = True,
        is_kit_override: bool = None, type_id_override: int = None
    ):
        usuario_seguro = usuario_id if hasattr(self, 'usuarios_validos') and usuario_id in self.usuarios_validos else 1
        ...
        if id_final not in self.mapa_ids_capa:
            ...
            self.servicos_mestre.append({
                "id": novo_id_capa, "status_id": 3, "movement_type_id": tipo_movimento_id, "contract_id": contrato_id,
                "user_id": usuario_seguro, "destination_order_id": None, "mode_transport_id": 1, "organization_id": organization_id,
                "recipient_customer_id": recipient_id, "deadline": mov_date, "details": details_capa, "situation": "APPROVED",
                "changed_by": usuario_seguro, "created_at": mov_date, "updated_at": mov_date, "deleted_at": deleted_at_mov
            })
            self.movimentos_mestre.append({
                "id": novo_id_capa, "number": id_final, "movement_date": mov_date, "service_order_id": novo_id_capa,
                "recipient_customer_id": recipient_id, "migrate_customer_id": None, "organization_id": organization_id,
                "status_id": 3, "created_by": usuario_seguro, "details": details_capa, "created_at": mov_date, "updated_at": mov_date, "deleted_at": deleted_at_mov
            })
        ...
        if tipo_movimento_id == 7 or not consumir_saldo:
            is_extra_flag, extra_id_atual, contrato_item_id_resolvido = False, None, int(contrato_item_id) if pd.notna(contrato_item_id) else None
        else:
            if is_kit_override is None and type_id_override is None:
                is_extra_flag, extra_id_atual, contrato_item_id_resolvido = self.calcular_saldo(contrato_item_id, recipient_id, equipment_id_ref, mov_date, item_servico_id_atual, fallback_contract_item_id, forcar_extra)
            else:
                is_extra_flag, extra_id_atual, contrato_item_id_resolvido = self.calcular_saldo(contrato_item_id, recipient_id, equipment_id_ref, mov_date, item_servico_id_atual, fallback_contract_item_id, forcar_extra, is_kit_override=is_kit_override, type_id_override=type_id_override)
        ...
        txt_detalhe_final = details_item if details_item else ("Item Extra (Saldo Esgotado)" if is_extra_flag else None)

        self.service_itens_mestre.append({
            "id": item_servico_id_atual, "status_id": 3, "service_order_id": id_capa_atual, "department_id": 2, "movement_type_id": tipo_movimento_id,
            "contract_item_id": contrato_item_id_resolvido, "alias": alias_item, "equipment_id": equipment_id_ref, "type_id": type_id_resolvido,
            "product_id": product_id_resolvido, "is_exchange": is_exchange, "is_extra": is_extra_flag, "quantity_product": None,
            "fulfilled_quantity_product": 0, "quantity": 1, "details": txt_detalhe_final, "address_id": cliente_final_address_id,
            "location_id": None, "created_at": mov_date, "updated_at": mov_date, "deleted_at": deleted_at_mov
        })

        item_mov_id_atual = self.mov_item_id_counter
        self.mov_item_id_counter += 1
        self.movimento_itens_mestre.append({
            "id": item_mov_id_atual, "movement_id": id_capa_atual, "movement_type_id": tipo_movimento_id, "service_order_item_id": item_servico_id_atual,
            "equipment_id": equipment_id_ref, "extra_id": extra_id_atual, "status_id": 3, "product_item_id": None, "alias": alias_movimento,
            "old_organization_id": None, "new_organization_id": organization_id, "operation_type": operation_type, "confirmed_at": None,
            "confirmed_by": None, "created_at": mov_date, "updated_at": mov_date, "deleted_at": deleted_at_mov
        })

        ship_id_atual = self.shipment_id_counter
        self.shipment_id_counter += 1
        self.shipments_mestre.append({"id": ship_id_atual, "status_id": status_shipment, "created_by": usuario_seguro, "created_at": mov_date, "updated_at": mov_date, "deleted_at": deleted_at_mov})
        
        shipment_item_id_atual = self.shipment_item_id_counter
        self.shipment_items_mestre.append({
            "id": self.shipment_item_id_counter, "shipment_id": ship_id_atual, "status_id": status_shipment, "movement_item_id": item_mov_id_atual,
            "volume_id": None, "details": f"Guia de transporte (Operação: {operation_type})", "address_id": cliente_final_address_id, 'created_at': mov_date, 'updated_at': mov_date
        })
        self.shipment_item_id_counter += 1
        self.shipment_movements_mestre.append({"shipment_id": ship_id_atual, "movement_id": id_capa_atual})

        # ==============================================================================
        # 🎯 FIX DEFINITIVO DO EQUIPAMENTO:
        # Passando o item_mov_id_atual E o endereço correto
        # ==============================================================================
    ...
    # ==============================================================================
    # 🎯 FIX DEFINITIVO DO EQUIPAMENTO: Atualização Física do Parque
    # ==============================================================================
        ...
        with self.engine_new.begin() as conn:
            ...
            for item in lista_dicionarios:
                if isinstance(item, dict) and "e_id" in item:
                    ...
                    # Fallback (Garante que scripts antigos não quebrem)
