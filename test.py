...
class MigracaoAluguel(BaseMigracaoMovimento):
    ...
    def executar(self):
        ...
        for _, row_csv in tqdm(df_csv.iterrows(), total=df_csv.shape[0], desc="Processando ALUGUEL"):
            ...
            (contrato_id_res, item_id_res, is_avulso, is_kit, is_excedente, teve_match_perfeito, motivo_divergencia) = self.aplicar_regras_comerciais_parquet(
            ...
            self.registrar_movimento(
                id_final=int(row_mov['id']), recipient_id=recipient_id,
                cliente_final_address_id=self.dados["dict_endereco_por_legacy_client"].get(cli_legado_id),
                usuario_id=usr_id, organization_id=int(org_id_resolvida),
                mov_date=dt_mov, deleted_at_mov=row_mov['deleted_at'] if pd.notna(row_mov['deleted_at']) else None,
                contrato_id=contrato_id_res, contrato_item_id=item_id_res, equipment_id_ref=equipment_id_ref,
                status_shipment=2, tipo_movimento_id=7 if is_avulso else 1, operation_type='AVULSO' if is_avulso else 'ALUGUEL',
                status_equipment_id=2, history_reason='SHIPPING_CONFIRMED_SEPARATE' if is_avulso else 'SHIPPING_CONFIRMED_RENT',
                forcar_extra=False, is_exchange=is_excedente, alias_item=str(row_csv.get('ITEM_DO_CONTRATO')).strip() if pd.notna(row_csv.get('ITEM_DO_CONTRATO')) else None,
                alias_movimento=row_csv.get('EQUIPAMENTO_NOME'), details_capa="Migração", details_item=detalhes_item
            )
